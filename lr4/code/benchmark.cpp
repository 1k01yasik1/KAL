...
struct Options {
  ...
  std::string graph_dir = "code/data/benchmark";
  size_t ants = 128;
...
Options ParseArgs(int argc, char** argv) {
  ...
  if (auto value = get("--graph-dir")) {
    options.graph_dir = *value;
  }
...
std::filesystem::path EnsureGraphFile(size_t vertices,
                                      unsigned int seed,
                                      size_t max_out_degree,
                                      const std::filesystem::path& directory) {
  std::filesystem::path path = directory / ("graph_" + std::to_string(vertices) + ".dot");
  if (!std::filesystem::exists(path)) {
    std::filesystem::create_directories(directory);
    std::string graphviz = GenerateGraphviz(vertices, seed, max_out_degree);
    std::ofstream out(path);
    if (!out) {
      throw std::runtime_error("Unable to create graph file: " + path.string());
    }
    out << graphviz;
    std::cout << "  Генерируем граф и сохраняем в: " << path.string() << std::endl;
  }
  return path;
}
...
int main(int argc, char** argv) {
  try {
    ...
    std::filesystem::path base_directory = options.graph_dir;
    ...
    for (size_t index = 0; index < options.sizes.size(); ++index) {
      ...
      std::filesystem::path graph_path =
          EnsureGraphFile(vertices, graph_seed, options.max_out_degree, base_directory);
      lr4::Graph graph = lr4::Graph::FromGraphvizFile(graph_path.string());
      std::cout << "  Используем файл: " << graph_path.string() << std::endl;
